...
**位置づけ**: [ARCHITECTURE_DESIGN.md](./0-全体アーキテクチャ設計.md) §2.3・§3.1 の詳細化。[ENGINE_SEPARATION_DESIGN.md](./1-エンジン分離設計.md) §3-B（sim 公開 API）・§3-D（スナップショット構造）と整合する。弐（Backend/DevOps）の Day 4〜9 の作業指示書に相当し、参（Frontend）のクライアント実装契約・肆（Gameplay）のマッチメイキング仕様の正本を兼ねる。
...
> **改訂（2026-07-11）**: 当初あった `hand`（RSP の選択手）フィールドは**削除**した（⑤ [BACKLOG.md](./5-バックログ.md) D-17）。手はリスポーン・自陣踏み込みで**サーバ権威**が変える状態であり、現行ルールにプレイヤーが手を選ぶ概念は存在しない。入力面に出すとチートと不整合の入口になるだけである。
