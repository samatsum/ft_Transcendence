...
壱（Engine）の Day 1〜5 と肆（Gameplay）の Day 4〜9 の作業指示書に相当する。
...
> `input_source`（`AI` / `EXTERNAL`）だけになりました。「担当: 壱が構造、肆がルール」は
...
>
> - 「壱（Engine）担当」「肆（Gameplay）担当」は当初の 4 レーン体制での分担案です。
...
壱の合計 ≒ 8.5人日、肆の合計 ≒ 6.5人日で、Day 1〜9 の2レーンに収まる（バッファ含む）。
