...
| CI 初回 green | オーナー（samatsum） | ⚠️ **未達**。billing ロックは解除されジョブは動くようになったが、2026-07-23 時点で全 7 実行が failure。`web-app`（TS）は success、`native` と `wasm` が失敗中（下記）。**「CI green ならマージ可」の運用（§7-2）はこれが解消するまで機能しない** |

**CI の現状（2026-07-23）**

| ジョブ | 状態 | 内容 |
|---|---|---|
| `web-app` | ✅ | npm ci → typecheck → build → backend 起動確認 |
| `native` | ❌ | `make` が exit 2。原因特定済み: minilibx が `test/mlx-test` もビルドし Linux では `-lbsd` を要求するため **`libbsd-dev` が必要**（README の必要パッケージと同じ）。ワークフローへ追加済み、次回実行で確認 |
| `wasm` | ❌ | `make web sim` は成功し `record.mjs` が exit 1。**原因未特定**（ログは API 経由で取得できず 403）。コンテナの PATH に `node` が無い可能性を疑い、`EMSDK_NODE` へのフォールバックと環境出力を追加して切り分け中 |
