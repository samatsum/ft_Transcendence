...
- **CI はまだ green になっていない**（§6）。billing ロックは解除済みだが `native` / `wasm` ジョブが失敗中で、原因の切り分けが進行中。
...
| CI 初回 green | オーナー（samatsum） | ⚠️ **未達だが解消途上**。billing ロック解除後も失敗が続いた。原因は2つとも特定・修正済みで、次回実行で green 見込み（下記）。**「CI green ならマージ可」の運用（§7-2）は green 達成後に有効化する** |

**CI で見つかった2つの問題（2026-07-23。いずれも原因特定・修正済み）**
