...
│   ├── engine_demo.html / engine_demo.js         # ローカルプレイの確認ページ（?map= と ?res= が使える）
...
├── app/{backend,frontend,shared}/    # Web アプリ（npm workspaces。W-01 で追加）
├── infra/                            # nginx・証明書・docker/（W-15）
...
`engine_demo.html` は `?map=rsp_map/rsp.cub` でマップを、`?res=1280x720` で内部解像度を切り替えられます。
