...
**Positioning**: This is the authoritative document, at a granularity that can be transcribed directly into GitHub Issues, integrating the W/F-series issues derived from [WS_PROTOCOL_DESIGN.md](./2-WSプロトコル設計.md) (②), [REST_API_DESIGN.md](./3-REST_API設計.md) (③), and [FRONTEND_DESIGN.md](./4-フロントエンド設計.md) (④) with the E/G-series issues in §6 of [ENGINE_SEPARATION_DESIGN.md](./1-エンジン分離設計.md) (①).
**torinoue (PM)** turns this into Issues and manages progress. Where acceptance-criteria detail exists in the individual design documents, it is referenced rather than repeated.
...
| G-01 | Combatant-unification structural change spec ([ENGINE_PHASE2_REPORT.md](../03_実装レポート/2-エンジンPhase2レポート.md)) | `3174497` (bundled with the design doc) |
...
| E-10 | **sim public API** (`game_create` through `game_destroy`, including `game_set_input_source`). ASan/LSan leak check OK across 3 RSP games × 1000 ticks. Pre-implemented the core of G-05 (making `target_score` part of match_rules, ②§4-B's 3–21) ([ENGINE_PHASE3_REPORT.md](../03_実装レポート/3-エンジンPhase3レポート.md)) | `6ee85f5` |
...
| E-14 | **CI** (`.github/workflows/ci.yml`): on every PR, native build + `make check` + `make test` (85 checks) + xvfb-driven smoke test across 5 maps + `make web sim` inside an emsdk container + record.mjs end-to-end + TS typecheck/build. **All jobs reached green on 2026-07-23** (fixed a missing `libbsd-dev` and a `type:module` regression). W-16 is planned to add FE lint ([ENGINE_E13_E14_REPORT.md](../03_実装レポート/4-エンジンE13E14レポート.md)) | `c068aa8`, `908529d` |
...
## 4. Backend / DevOps lane (torinoue + samatsum) W-01–W-16 — new

Numbered items in the acceptance-criteria column indicate mapping to acceptance criteria 1–6 in ②§10.

| Issue | Title | Acceptance criteria | Dependency | Day (14-day schedule; for the 5-day schedule see [⑥§5.1](./6-チーム分担計画.md)) |
|---|---|---|---|---|
...
| W-10 | GameRoom + `sim.wasm` integration (②§6; 30Hz tick, even-tick broadcast, state machine) | Multiple rooms run concurrently in Node. Tick-overload warning logs work | E-11 | 4–5 |
| W-11 | Game WS (join/input/leave/welcome/snapshot/event; ②§5, reflecting D-17's revision) | **№6**: invalid messages (schema violations, over 4KB, seq going backwards, out-of-range rules) are discarded/errored per spec. **№5**: snapshot size measured < 1KB | W-10 | 5–6 |
...
## 5. Frontend lane (mamiyaza + hminemur) F-01–F-12 — new

| Issue | Title | Acceptance criteria | Dependency | Day (14-day schedule; for the 5-day schedule see [⑥§5.1](./6-チーム分担計画.md)) |
|---|---|---|---|---|
| F-01 | Scaffold (Vite/React/TS/Tailwind/Router/ErrorBoundary/StrictMode) | Foundation for ④§6-1's zero-console-errors policy. Zero build warnings | W-01 | 1 |
| F-02 | fetch wrapper + shared zod integration (③§1 error-envelope handling, toast integration) | 401 redirects to `/login`; errors appear as toasts | F-01 | 3 |
...
| F-06 | GameView integration (render.wasm loading, Canvas, interpolation receiver, input sending; wired to E-08/E-12) | A match works between 2 browsers and the spectator view doesn't break | F-05, E-12, W-11 | 6–7 |
| F-07 | Full HUD overlay suite (④§3.3's 8 elements) | ④§6-6 (grace→AI transition is displayed) | F-06 | 7–8 |
...
| Gate 2 (Day 7 → **Day 3 under the 5-day schedule**) | **A 2v2 RSP match is playable start to finish between 2 browsers** | E-08–E-12 / G-01–G-05 / W-08–W-11 / F-05–F-08 | Not yet (**E, G, W-08 core, and W-09–W-12 are done**. What remains on the server side is W-08 integration with W-04/W-05; on the frontend side, F-05–F-08)<br>**W-12 (disconnect/reconnect/AI takeover) is not counted toward Gate 2 — it was carried out and completed on Day 4 instead** (consistent with [⑥§5.1](./6-チーム分担計画.md)). W-12 is ②§10-B №4 and is not something to be dropped |
...
|---|---|---|---|---|
| 1 | Web game | 2 | RSP works as a real match | Confirmed at Gate 2 |
| 2 | Remote players | 2 | Cross-browser match + disconnect/reconnect (W-12) | Gate 2 + (W-12 carried out on Day 4) |
| 3 | 3+ player multiplayer | 2 | **4 humans in the same match** (no AI seats) | Confirmed at Gate 2 |
| 4 | Frameworks on both FE/BE | 2 | React + Fastify demonstrable in code | Confirmed at Gate 2 |
...
In parallel, F-01→F-05 (mamiyaza) and F-06→F-08 (hminemur) converge on Gate 2.
The C side (E-series, G-series) is entirely Done; only the TypeScript Backend/DevOps and Frontend lanes remain.
See [6-チーム分担計画](./6-チーム分担計画.md) for the assignment of the 4 team members.

## 7. Upstream Process Closure Declaration

- Design documents ①–⑤ are complete, and the trace now holds end-to-end from
  requirements ([ft_トランセンデンス.md](../01_課題/ft_トランセンデンス.md)) → architecture (ARCHITECTURE) →
