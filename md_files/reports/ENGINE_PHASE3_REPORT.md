...
対象コミット: E-10=`6ee85f5` / E-11=`21bc5c2` / E-12=`55c5a82`（2026-07-16）
...
| 3 | **FPS モードのヘッドレスは未完成**: 射撃（render の depth バッファ依存）・ゴール/収集判定（カメラ基準の check_quest）・追跡 AI（カメラ座標参照）・接触死（プレイヤーのみ）が「ローカルプレイヤー席」前提のまま。G-06〜G-08（肆担当）で戦闘員基準へ一般化が必要。E-10 の席生成・入力・snapshot 自体は FPS でも動く | 依存 Issue 待ち |
