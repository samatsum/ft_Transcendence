...
| 10 | Custom-made design system | Web Minor | 1 | The subject requires 10+ reusable components plus a color palette, typography and icons. `app/frontend/src/components/` has 12 `.tsx` files, but only **9 are UI components** (Button, Card, Modal, Toast, FormField, Input, Header, Footer, Layout). The other three — `ErrorBoundary`, `RequireAuth`, `RedirectIfAuth` — are **not reusable UI parts**: `ErrorBoundary` is a single-use error screen, and the other two are route guards (`RequireAuth` renders only a loading placeholder, `RedirectIfAuth` renders no markup at all). They do render styled markup, so the exclusion is about *reusability*, not about whether they draw anything, so counting them toward a *design system* would not survive an evaluator's question. One more real UI component is needed, plus the palette / typography / icon set, which is the larger remaining half |
...
|---|---|---|---|
| 1 | **Another game (FPS) with history + matchmaking** | Gaming Major 2 | Matchmaking is shared with RSP (B-09 done; **B-08 is core-complete and still needs the B-04/B-05 auth integration**). **B-13 is necessary but not sufficient**: FPS also has one remaining unfixed defect, **G-12** (remote players have no appearance) — **G-11** (shooting could eliminate the other seat) was fixed 2026-08-09, so restoring this module now means B-13 + G-12. Still the cheapest restore — B-13 also revives #2 below — budget two issues, not three |
