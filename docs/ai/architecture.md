...
|---|---|---|---|
| 1 | **Another game (FPS) with history + matchmaking** | Gaming Major 2 | Matchmaking is shared with RSP (B-09 done; **B-08 is core-complete and still needs the B-04/B-05 auth integration**). **B-13 is necessary but not sufficient**: FPS also has two unfixed defects, **G-11** (shooting can eliminate the other seat) and **G-12** (remote players have no appearance), so restoring this module means B-13 + G-11 + G-12. Still the cheapest restore — B-13 also revives #2 below — but budget three issues, not one |
