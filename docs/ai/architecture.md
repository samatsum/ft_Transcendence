...
>   (samatsum, through 2026-08-09), and **team operation kicked off 2026-08-08** (role assignment took
>   effect at the 2026-08-23 meeting): the project is now a 6-person team (PO/PM/TL/2 Developers/1
>   Supporter). Authoritative source:
...
- **A4**: "FPS 1v1 race" is defined as: two players spawn simultaneously on the same map, and whoever reaches the goal cell first wins. The existing "collect item → open door" mechanic is reused as a gate to the goal, and the existing enemy (M) functions as an obstruction hazard. **Update 2026-08-09 (G-11)**: shooting the other seat is now a legitimate tactical delay — hp (default 3, per-map via `.cub` `PH`) reaching 0 sends the seat through the same hazard-contact death/respawn cycle rather than ending the match. The win condition itself is unchanged; elimination is not a second way to win.
...
| 10 | Custom-made design system | Web Minor | 1 | **Complete** (2026-08-20). The subject requires 10+ reusable components plus a color palette, typography and icons. `app/frontend/src/components/` now has **18 reusable UI components**: the original 9 (Button, Card, Modal, Toast, FormField, Input, Header, Footer, Layout), `Select` / `LinkButton` / `Alert` (added in #220; the count was 15 at completion), plus 5 icon components in `app/frontend/src/components/icons/` (CloseIcon, InfoCircleIcon, CheckCircleIcon, ExclamationCircleIcon, WarningTriangleIcon, built on a shared stroke-based `IconBase` rather than an external icon library) plus `ColorSwatch` (`ErrorBoundary`, `RequireAuth`, `RedirectIfAuth` are still excluded — same reusability argument as before). Typography was tokenized into 6 semantic classes (`text-heading-lg/md/sm`, `text-label`, `text-body`, `text-caption`) in `index.css`'s `@theme`, alongside the pre-existing color palette. Everything is catalogued in `components/design-system.ts` (a `DesignSystem` namespace object for editor autocompletion) and a dev-only gallery page at `/dev/design-system` (`pages/DesignSystemPage.tsx`, excluded from the production build via `import.meta.env.DEV`). Verified via `npm run build` (tsc + vite build) and a manual browser check of the catalog page |
| 11 | Spectator mode | Gaming Minor | 1 | GV-06 already handles `welcome.role === 'spectator'` (input suppressed, "観戦中" shown). Remaining: the WS `spectate` action (currently a `not_participant` stub → **B-17**) and the viewpoint-switch UI (**GV-12**). **This is now the largest of the two remaining bonus gaps** — the other is #12 (the status page plus backup & recovery); #10 (design system) is complete as of 2026-08-20. Declaring it makes both issues mandatory: an incomplete declared module scores 0 (Chapter VII) |
...
|---|---|---|---|
| 1 | **Another game (FPS) with history + matchmaking** | Gaming Major 2 | Matchmaking is shared with RSP (B-09 done; **B-08 is core-complete and still needs the B-04/B-05 auth integration**). FPS's two post-backlog defects — **G-11** (shooting could eliminate the other seat) and **G-12** (remote players had no appearance) — were both fixed 2026-08-09, so restoring this module now needs only **B-13**. Still the cheapest restore — it also revives #2 below |
...
design system (1pt) is now complete (§4.2). Health check (1pt) already has its endpoint. None of
...
> single-contributor period, and was replaced by a new 6-person team, kicked off 2026-08-08, under different names.
