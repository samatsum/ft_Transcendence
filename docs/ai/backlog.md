...
| B-03 | Prisma + SQLite schema v1 (the 5 tables in ③§3) + migration ([Issue #77](https://github.com/samatsum/ft_Transcendence/issues/77)) | `prisma migrate` is wired into the compose first-run | I-01 | 2 |
| B-04 | Full auth suite (signup/login/logout/me, argon2id, session cookie, ③ D-4–D-8) ([Issue #78](https://github.com/samatsum/ft_Transcendence/issues/78)) | All ③§2-A behaviors + login-failure responses are indistinguishable | B-02, B-03 | 2–3 |
| B-05 | Origin validation (REST mutating routes + WS upgrade) and shared cookie authorization ([Issue #79](https://github.com/samatsum/ft_Transcendence/issues/79)) | Mutating routes/WS from a different Origin are rejected | B-04 | 3 |
...
| I-16 | CI (integrates E-14: 3-target build + `make check` + native smoke + FE lint/typecheck) ([Issue #82](https://github.com/samatsum/ft_Transcendence/issues/82)) | Runs automatically on every PR | I-01, E-05, E-11 | 3 |
| **B-17 (new, 2026-08-08)** | Server side of `spectate` — accept a non-participant connection, answer `welcome` with `role: "spectator"`, and include it in the snapshot fan-out ([Issue #80](https://github.com/samatsum/ft_Transcendence/issues/80)) | A third browser connects to a live match and receives snapshots without occupying a seat; the match's player count and AI-takeover logic are unaffected by spectators | B-11 | 11 |
...
| GV-08 | Match transition flow (match_found auto-transition → countdown → match_end modal → back to lobby) ([Issue #87](https://github.com/samatsum/ft_Transcendence/issues/87)) | No console errors while the WS connection is re-established mid-transition | F-05, GV-07 | 7–8 |
...
| GV-12 **(now required, 2026-08-08)** | Spectator UI (④§3.3 spectator HUD) ([Issue #88](https://github.com/samatsum/ft_Transcendence/issues/88)) | A non-participant can watch a live match through to `match_end` without console errors, and the spectator HUD does not expose either seat's private input | GV-07, **B-17** | 11 |
