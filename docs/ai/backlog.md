...
Historically the PM role transcribed this into Issues and tracked progress; after a 2026-08-05 dissolution and single-contributor period, team operation kicked off 2026-08-08 with torinoue back as PM (see [`../human/はじめに/チーム体制.html`](../human/はじめに/チーム体制.html) for the current roster). Where acceptance-criteria detail exists in the individual design documents, it is referenced rather than repeated.
...
### 3.1 New gaps found during FPS 1v1 doc review (discovered 2026-08-07; G-11 and G-12 both done 2026-08-09)
...
|---|---|---|---|
| G-11 (done) | FPS's inherited single-player shoot mechanic could eliminate the other seat ([Issue #46](https://github.com/samatsum/ft_Transcendence/issues/46)) | **Resolution changed from the original proposal**: the issue as filed suggested skipping `is_player` nodes in `damage_enemy`, but reading the code showed `is_player` is never set on the server-authoritative path (`sim_api.c`'s `game_add_combatant`) — it's exclusive to the native single-player `create_player_combatant`. The actual discriminator is `is_hazard`. **samatsum's design call**: shooting between seats is now allowed and meaningful — seats get a configurable `player_hp` (`.cub` `PH`, default 3, independent of hazard `enemy_hp`), and hp reaching 0 no longer `delete_enemy`s the seat; it sets `death_timer` exactly like a hazard-contact death (G-08) and the existing `update_death` → `mode_ops.respawn` path returns it to its own spawn. The win condition is unchanged ("collect → door → goal" only); elimination is a tactical delay, not a second win path. A dead seat (`death_timer > 0`) takes no further damage. `death_timer` was already part of the snapshot format, so no ② WS/HUD change was needed. Verified by 3 new `make test` cases ([sim_test.c](../../codes/tests/sim_test.c)): hazards are still deletable by shooting, a seat survives lethal shots and respawns with hp restored, a dead seat ignores further shots. **Note (2026-09-17)**: those tests call `damage_enemy` directly — in an online match no seat could actually fire until [Issue #187](https://github.com/samatsum/ft_Transcendence/issues/187), because `act` was fixed at 0 and the sim had no shot path for seats. #187 added `shoot_from_combatant` and wired `act` bit0 through backend and frontend | G-06, G-08 |
| G-12 (done) | FPS gave the other seat no visual appearance ([Issue #47](https://github.com/samatsum/ft_Transcendence/issues/47)) | **Design decision changed from the original proposal**: the issue as filed wanted the opponent rendered "visually distinct from a hazard enemy." **samatsum's design call**: the disguise is deliberate and total instead — diegetic justification is "you're a survivor hallucinating from fear; everyone looks like a Monster." The opponent renders as **fully indistinguishable from a hazard by appearance alone**; no recolor, no marker, no partial tell. The only legitimate way to identify a real opponent is behavioral (AI movement patterns vs. player-driven movement), which is out of scope for this Issue. Root cause was [`apply_remote_combatant`](../../codes/srcs/platform/web/web_snapshot.c) only assigning a sprite texture inside `if (game->mode == MODE_RSP)` (the team/hand icon swap) — no equivalent branch for `MODE_FPS`, so a remote FPS seat's display node kept whatever texture it was created with and was never billboard-rotated. **Fix**: reuse [`fps_enemy_assets.c`](../../codes/srcs/fps/enemy/fps_enemy_assets.c)'s existing `enemy_tex[]` (8-direction) and `update_texture()` (the same camera-relative-angle → texture-index math already used for hazard AI) directly on the remote node inside a new `else if (game->mode == MODE_FPS)` branch — no new texture asset needed; `dir_angle` was already carried by the snapshot and already applied to the node, only the texture pick was missing. **Verified visually**, not just by code reading: extended `web/sim_demo/record.mjs` to accept an `fps` mode (writes `snapshots_fps.json`, a fixed 20s recording since FPS doesn't finish deterministically from scripted wandering input — the point is watching the render, not the match outcome) and confirmed via a headless-Chromium screenshot of `replay.html?feed=snapshots_fps.json` that the AI seat renders as a Monster, indistinguishable up close. **While investigating this, also confirmed RSP already does the analogous thing correctly** — the `MODE_RSP` branch sets `node->sprite->tex` from *that node's own* `rsp.team`/`rsp.hand` (not the viewer's), so "what the opponent sees of me" and "what I see of the opponent" go through the identical code path and are structurally symmetric by construction. This RSP part is verified by reading the code only; **not yet verified with a real 2-browser session** — same class of caveat as GV-06 (`make test` doesn't compile `web_snapshot.c`, and the dev-auth stub blocks a real 2-browser session). Re-verify once B-04 + F-05 + B-09 land, same as GV-06 | E-12, G-08 |
...
> was the sole contributor through the single-contributor period. Team operation kicked off 2026-08-08
...
| Issue | Title | Acceptance criteria | Dependency |
|---|---|---|---|
| I-01 (done) | Add repository skeleton (`app/backend/`, `app/frontend/`, `app/shared/`, `infra/`, TS tooling, `env.example`+`.gitignore`) | backend/frontend start up under D-18's layout (keep current layout + add) → **achieved** (§1) | — |
...
| B-03 (done) | Prisma + SQLite schema v1 (the 5 tables in ③§3) + migration ([Issue #77](https://github.com/samatsum/ft_Transcendence/issues/77)) | **Acceptance rewritten 2026-08-09.** It used to read "`prisma migrate` is wired into the compose first-run", which B-03 could never satisfy on its own — **there is no compose app service to wire it into until I-15**, so the row would have stayed open indefinitely. The compose half moved to the I-15 row; B-03's own bar is now **"the migration applies to an empty database and produces ③§3's schema"**, checked by `npm run check:db` (6 checks: `migrate deploy` on a fresh file, the 5 tables exist, an AI seat with `userId=null` is insertable, `email`/`displayNameLower` uniqueness rejects duplicates, `(matchId, slot)` is unique, and deleting a `User` cascades its `Session` while leaving `MatchPlayer` with a null `userId`). **All 5 tables were built even though 3 are unused** (`Friendship` → B-07, `Match`/`MatchPlayer` → B-13, all not declared) — decision and rationale in [Issue #93](https://github.com/samatsum/ft_Transcendence/issues/93). **③§3 gained one column**: `displayNameLower`, because a Prisma/SQLite schema cannot declare the lower-cased index that §1-B's case-insensitive uniqueness needed. Prisma 7 also forced two structural choices — `datasource.url` is gone from the schema (it lives in `prisma.config.ts`) and `PrismaClient` requires the `@prisma/adapter-better-sqlite3` driver adapter. The server does **not** open a connection at boot: `src/db/client.ts` only exports a factory, so CI's health-check job still starts without a database ([PR #94](https://github.com/samatsum/ft_Transcendence/pull/94)) | I-01 |
| B-04 | Full auth suite (signup/login/logout/me, argon2id, session cookie, ③ D-4–D-8) ([Issue #78](https://github.com/samatsum/ft_Transcendence/issues/78)) | All ③§2-A behaviors + login-failure responses are indistinguishable | B-02, B-03 |
| B-05 | Origin validation (REST mutating routes + WS upgrade) and shared cookie authorization ([Issue #79](https://github.com/samatsum/ft_Transcendence/issues/79)) | Mutating routes/WS from a different Origin are rejected | B-04 |
| B-06 **(not declared, 2026-08-08)** | Avatar upload + nginx static serving (③§2-B) | Files over 2MB, spoofed Content-Type, or invalid magic bytes are rejected with 413/415 | B-04, I-15 |
| B-07 **(not declared, 2026-08-08)** | Full friends API (③§2-C) | Duplicate bidirectional requests / self-requests return 409. Presence composition is returned | B-04, B-08 |
...
| B-09 (done) | MatchPlan → GameRoom creation → token commit → `match_found` + context release on lifecycle end (②§4-C–E) | **№2 achieved**: `npm run check:lobby` confirms real GameRoom creation across all 3 paths (manual/60-second/full-room). Automatically checks: 10-second connection wait, AI-ization of unconnected seats, context release when human count is 0 at close, rollback on creation failure/5-second timeout, GameRoom discard on delayed success, and zero leftover reservations | B-08, B-10 |
| B-10 (done) | GameRoom + `sim.wasm` integration (②§6; 30Hz tick, even-tick broadcast, state machine) | Multiple rooms run concurrently in Node. Tick-overload warning logs work | E-11 |
| B-11 (done) | Game WS (join/input/leave/welcome/snapshot/event; ②§5, reflecting D-17's revision) | **№6**: invalid messages (schema violations, over 4KB, seq going backwards, out-of-range rules) are discarded/errored per spec. **№5**: snapshot size measured < 1KB | B-10 |
...
| B-13 **(not declared, 2026-08-08 — restore candidate №1)** | Match persistence + `match_result` broadcast + history/stats API (②§6-C, ③§2-D) | **№1**: end-to-end from match creation → decision → DB row → lobby receives it. **№3**: a custom room with `target_score=3` decides at 3 points | B-11, B-03, B-09 |
| B-14 (done) | `GET /api/maps` + `welcome.map_text` path (③§2-E) | What the server loads and what it distributes as text always match | B-10 |
| I-15 | Docker Compose + nginx TLS + single-command startup (ARCHITECTURE §2.4; first run includes cert generation, migration, and `make web`-equivalent asset conversion) ([Issue #81](https://github.com/samatsum/ft_Transcendence/issues/81)) | `git clone` into an empty folder → `docker compose up` → HTTPS connection from Chrome (measured per §9.1).<br>**Moved here from B-03 (2026-08-09)**: **`prisma migrate deploy` must run on the compose first-run**, so a fresh clone comes up with the schema already applied. B-03 delivered the migration itself and `npm run db:deploy`; what remains is invoking it from the container's startup path and pointing `DATABASE_URL` at a persisted volume rather than `app/backend/data/`.<br>**Addition (2026-07-27, TL proposal)**: **`sim.wasm` and `render.wasm` must be generated by `docker compose up` alone, starting from an empty `web/build/`.** No assumption that the host has `emcc` — evaluator machines do not have Emscripten installed. Wire the existing `engine-build` service (`infra/docker/engine-build/Dockerfile`), which runs `make web sim`, directly into the startup path. Also ensure **generated artifacts are not root-owned on the host side when `HOST_UID`/`HOST_GID` are set** — `docker-compose.yml`'s `user:` currently defaults to `0:0` (root) if they're left unset, so this criterion is opt-in, not automatic; I-15 should decide whether to auto-detect the host UID/GID instead of requiring the user to set them (see `.env.example`, `infra/README.md`) | I-01 |
...
| Issue | Title | Acceptance criteria | Dependency |
|---|---|---|---|
| F-01 (done) | Scaffold (Vite/React/TS/Tailwind/Router/ErrorBoundary/StrictMode) | Foundation for ④§6-1's zero-console-errors policy. Zero build warnings | I-01 |
| F-02 (done) | fetch wrapper + shared zod integration (③§1 error-envelope handling, toast integration) | 401 redirects to `/login`; errors appear as toasts | F-01 |
| F-03 | Auth screens + route guard (④§3.1, §1) ([Issue #83](https://github.com/samatsum/ft_Transcendence/issues/83)) | ④§6-3 | F-02, B-04 |
| F-04 | Common layout + **actual Privacy/ToS copy** (④§2, §3.5) ([Issue #84](https://github.com/samatsum/ft_Transcendence/issues/84)) | ④§6-4. Zero placeholder text | F-01 |
...
| GV-12 **(now required, 2026-08-08)** | Spectator UI (④§3.3 spectator HUD) ([Issue #88](https://github.com/samatsum/ft_Transcendence/issues/88)) | A non-participant can watch a live match through to `match_end` without console errors, and the spectator HUD does not expose either seat's private input | GV-07, **B-17** |
...
**I-01/B-02/B-03/B-09/B-10/B-11/B-12/B-14 are done**, so the remaining work on the game-server side is
**B-04→B-05→B-08** (auth, Origin, and the final integration of real cookie
