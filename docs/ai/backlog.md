...
| B-06 | Avatar upload + nginx static serving (③§2-B) | Files over 2MB, spoofed Content-Type, or invalid magic bytes are rejected with 413/415 | B-04, I-15 | 10 |
| B-07 | Full friends API (③§2-C) | Duplicate bidirectional requests / self-requests return 409. Presence composition is returned | B-04, B-08 | 10 |
...
| B-13 | Match persistence + `match_result` broadcast + history/stats API (②§6-C, ③§2-D) | **№1**: end-to-end from match creation → decision → DB row → lobby receives it. **№3**: a custom room with `target_score=3` decides at 3 points | B-11, B-03, B-09 | 8–9 |
...
> F-03–F-05 and GV-08・F-09・F-10・F-11・GV-12 remain unassigned and not started (未完成), open for a future team member. See
...
| F-09 | Profile/stats/history + self-editing + avatar (④§3.4) | Files over 2MB show an immediate error. win_rate matches ③§2-D | F-02, B-13, B-06 | 9–10 |
| F-10 | Friends UI (④§3.2 friends region + ④§3.4 buttons) | Presence badge updates in real time | F-05, B-07 | 10 |
| F-11 | Responsive + accessibility adjustments (④ D-13, §6-2/7) | Confirmed at 375px full-screen. Reachable up to match start using keyboard only | F-04・F-05・GV-06・GV-07・GV-08・F-09・F-10 | 11 |
| GV-12 | (reserve 1) Spectator UI (④§3.3 spectator HUD) | Only undertaken under the conditions in ARCHITECTURE §4.3 | GV-07 | 11 |
...
| Gate 3 (Day 11 → **Day 4 under the 5-day schedule**) | Core 14pt fully working in the integrated environment | B-13, B-14, F-09–F-11, G-06–G-09 (FPS-related), and everything else | Not yet (**G-06–G-09 are done**) |
...
| 9/10/11/12 | Bonus | +5 | Only declare items that are **fully working** (never declare something that doesn't work) | Advanced 3D (done, needs README justification) / design system (9 of 10 components exist) / spectator (GV-06 path exists) / health-check page (`/api/health` exists) |
...
**I-01/B-09/B-10/B-11/B-12/B-14 are done**, so the remaining work on the game-server side is now
**B-13 persistence-integration support** after B-03 (plus the final B-08 integration of auth/Origin via B-02→B-05→B-08).
