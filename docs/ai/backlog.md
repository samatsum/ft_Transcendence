...
### 3.1 New gaps found during FPS 1v1 doc review (discovered 2026-08-07; G-11 done 2026-08-09, G-12 unassigned)
...
| G-12 | FPS gives the other seat no visual appearance ([Issue #47](https://github.com/samatsum/ft_Transcendence/issues/47)) | **Design decision changed from the original proposal (2026-08-09)**: the issue as filed wanted the opponent rendered "visually distinct from a hazard enemy." **samatsum's design call**: make the disguise deliberate and total instead — diegetic justification is "you're a survivor hallucinating from fear; everyone looks like a Monster." The opponent must render as **fully indistinguishable from a hazard by appearance alone**; no recolor, no marker, no partial tell. The only legitimate way to identify a real opponent is behavioral (AI movement patterns vs. player-driven movement), which is out of scope for this Issue. Root-cause diagnosis unchanged: [`apply_remote_combatant`](../../codes/srcs/platform/web/web_snapshot.c) only assigns a sprite texture inside `if (game->mode == MODE_RSP)` (the team/hand icon swap) — there is no equivalent branch for `MODE_FPS`, so a remote FPS seat's display node keeps whatever texture it was created with and is never billboard-rotated. **Planned approach**: reuse [`fps_enemy_assets.c`](../../codes/srcs/fps/enemy/fps_enemy_assets.c)'s existing `enemy_tex[]` (8-direction) and `update_texture()` (the same camera-relative-angle → texture-index math already used for hazard AI) directly on the remote node inside `apply_remote_combatant` — no new texture asset needed; the direction data (`dir_angle`) is already carried by the snapshot (`buf[5]`/`entry[5]`) and already applied to the node (line 214), it's only the texture pick that's missing. Verify manually with 2 browsers: the FPS opponent cannot be told apart from a hazard 'M' by appearance. **While investigating this, also confirmed RSP already does the analogous thing correctly** — the `MODE_RSP` branch sets `node->sprite->tex` from *that node's own* `rsp.team`/`rsp.hand` (not the viewer's), so "what the opponent sees of me" and "what I see of the opponent" go through the identical code path and are structurally symmetric by construction. Verified by reading the code only; **not yet verified with 2 real browsers** — same class of caveat as GV-06 ("acceptance not currently reproducible": `make test` doesn't compile `web_snapshot.c`, and the dev-auth stub blocks a real 2-browser session). Re-verify once B-04 + F-05 + B-09 land, same as GV-06 | E-12, G-08 |
