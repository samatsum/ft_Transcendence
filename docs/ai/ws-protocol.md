...
| `act` | uint4 bitmask (optional; omitted = 0) | bit0=fire (the trigger is held) / bit1–3=reserved, accepted and ignored. Like `mv` it carries **held state**, so a held trigger sends 1 every message; the fire interval (0.5 s cooldown, `SEAT_SHOT_COOLDOWN`) is owned by the sim seat, not the client. Firing only has an effect in FPS (`mode_ops.can_shoot`); RSP ignores the bit. Revised by [Issue #187](https://github.com/samatsum/ft_Transcendence/issues/187) — see the note below |

> **Revision (2026-09-17, [Issue #187](https://github.com/samatsum/ft_Transcendence/issues/187))**: `act` was previously "fixed at 0 for both modes", which contradicted G-11 (⑤ [BACKLOG.md](./backlog.md): shooting between seats is allowed and meaningful) and the player-facing controls (`Space` = fire). The tech lead chose to make firing work. The server-side hit test is **not** the native `shoot_target` (that one needs the local camera and the render depth buffer, neither of which exists in the sim); `shoot_from_combatant` casts a straight line from the seat along its `yaw`, stops at the first wall/door, and damages the nearest living combatant within 0.5 cells of the line.
>
...
| tick overload | A warning is logged (candidate metric for monitoring stretch-item 4, 監視保4) if `game_step` takes over 50% of its period. No cap is placed on room count (reasoning: unnecessary at the evaluation scale of 4 players × a few rooms — **not yet load-tested**; this is one of Gate 2's pass criteria in [backlog.md §6.1](./backlog.md), which is not yet met) |
