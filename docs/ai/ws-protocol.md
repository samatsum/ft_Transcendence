...
| `act` | uint4 bitmask (optional; omitted = 0) | bit0=fire (the trigger is held) / bit1–3=reserved, accepted and ignored. Like `mv` it carries **held state**, so a held trigger sends 1 every message; the fire interval (1 s cooldown) is owned by the sim seat, not the client. Firing only has an effect in FPS (`mode_ops.can_shoot`); RSP ignores the bit. Revised by [Issue #187](https://github.com/samatsum/ft_Transcendence/issues/187) — see the note below |
