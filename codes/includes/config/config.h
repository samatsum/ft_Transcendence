...
// ビット0は初期状態で収集物だったセルを保持する。data は収集・訪問で 'A' に
// 書き換わるため、snapshot で収集済み座標を復元するには別レイヤが必要になる
# define CELL_COLLECTIBLE		(1 << 0)
...
// player_hp は席（プレイヤー）専用の別枠で、.cub の PH で上書き。G-11）
