...
だから片方（`web/engine_demo.js`）で分かったことは、もう片方（`record.mjs`）でも通用します。
...
- 呼び出しの実例: [web/sim_demo/record.mjs](../../web/sim_demo/record.mjs)（Node → sim.wasm）/ [web/engine_demo.js](../../web/engine_demo.js)（ブラウザ → render.wasm）
