...
**対象（必須）**: **samatsum**（テクニカルリード／エンジン境界の説明・C言語 側の保守）。口頭説明の台本としても使う。  
**対象（任意）**: 他メンバー。実装の必須読書ではない（各人の `onboarding_docs/reading_guide_*.md` に従う）。知るなら §1〜§3 だけで足りる。torinoue / mamiyaza / hminemur への実務的な影響は **§6**。  
