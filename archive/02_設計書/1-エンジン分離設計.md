...
**位置づけ**: [ARCHITECTURE_DESIGN.md](./0-全体アーキテクチャ設計.md) §2.1〜2.2 の詳細化。
Engine レーンの Day 1〜5 と Gameplay レーンの Day 4〜9 の作業指示書に相当する（**両レーンとも samatsum が実施し完了済み**）。
**原則**: ネイティブ版（MiniLibX/X11）を**常に動く状態に保ちながら**段階的に分離する。
本書は実装コードを含まない（インターフェース仕様と受入条件のみ）。

---

> ## ✅ 実装状況（2026-07-23 時点）
>
> **本書に書かれた作業は、全て完了しています。**
> 予定表・受入条件の形で書かれていますが、これは着手前に作った作業指示書だからで、
> 「これからやること」ではありません。
>
> | 節 | 内容 | 状況 |
> |---|---|---|
> | §1 | 3層 × 3ビルドターゲット | **完了**。`make` / `make web` / `make sim` の 3 つが全て通り、稼働中 |
> | §2 | プラットフォーム層 `pf_*` | **完了**。`mlx.h` を include するファイルは `platform/native/platform_native.c` の **1 本だけ**（sim 層・render 層はゼロ）。退行は CI の `web`/`sim` ビルドが検知する（MLX ヘッダの無い環境でコンパイルするため） |
> | §3 | sim 公開 API（`game_step` 他） | **完了**。正本は [`codes/includes/platform/sim.h`](../../codes/includes/platform/sim.h)。設計時から増えた API は下の「§3-B の実装との差分」を参照 |
> | §4 | アセットパイプライン / `.cub` メモリリーダ / ビルド | **完了**。§4-C のゲームルール変更（G-05〜G-10）も全て実装済み |
> | §5 | 段階的移行手順 S1〜S7 | **完了**（S1〜S7 すべて）。native は全期間を通じて 1 度も壊さずに到達 |
> | §6 | Issue バックログ E-01〜E-14 / G-01〜G-10 | **全 24 件クローズ**。表の各行に ✅ を付けてあります |
>
> **エンジン（C）側に残作業はありません。**
> 残っているのは TypeScript のサーバとフロントエンド（W-02〜W-16 / F-01〜F-12）だけです。
>
> - 実装の詳細な記録: [3-エンジンPhase3レポート](../03_実装レポート/3-エンジンPhase3レポート.md)
>   / [4-エンジンE13E14レポート](../03_実装レポート/4-エンジンE13E14レポート.md)
> - 次にこのエンジンを使う人（W-10 担当）への申し送り:
>   [3-エンジンPhase3レポート](../03_実装レポート/3-エンジンPhase3レポート.md) の「W-10 への申し送り」
> - 全体の進捗: [5-バックログ](./5-バックログ.md) / 担当: [6-チーム分担計画](./6-チーム分担計画.md)

---

## 0. 前提（コード実査の結果）

- **最新 HEAD（ルートの `codes/srcs/{common,fps,rsp}`）で検証済み**。
  `t_game` は `t_fps_data fps` / `t_rsp_data rsp` のモード別集約 + `t_mode_ops` ディスパッチを持つ
  DEV_DOC 記載どおりの構成であることをソースで確認した。
- 実査で確認した「分離に有利な事実」:
...
> **✅ 3 ターゲットとも完成し、全て稼働しています。**
> 「完了ゲート」列の Day 表記は着手前の目安です。ゲート1 は 2026-07-11 に通過しました。
> ビルドコマンドは `make`（native） / `make web`（`render.wasm`） / `make sim`（`sim.wasm`）で、
> [CI](../../.github/workflows/ci.yml) が毎 push で 3 つとも自動ビルドしています。

---

## 2. プラットフォーム層インターフェース仕様（`pf_*`）

新規ヘッダ `includes/platform/platform.h` に集約。
**sim 層・render 層は MLX ヘッダを一切 include しない**ことが完了条件。
...
- `t_framebuffer` は「ピクセルポインタ・幅・高さ・1行バイト数」のみの新設構造体。
  既存 `t_window.screen`（`t_image`）の役割を置き換え、
  `draw_pixel` はこの構造体を参照する形に変更する（変更は参照先の型だけで、ロジックは不変）。
- **ピクセルフォーマットの罠**: MLX は BGRA（リトルエンディアン int 書き込み）、Canvas の `ImageData` は RGBA。
  変換は web 側 `pf_present` 相当の JS で一括吸収するか、ビルドフラグでチャンネル順を切り替える。
  **採用: JS 側で吸収**（C コードを両ターゲットで完全同一に保つ）。
- ループ駆動の逆転: 現行は `mlx_loop` がコールバックを呼ぶ（push型）。
  web は `requestAnimationFrame`、サーバは `setInterval` が
  **`main_loop` 相当の1回分関数を外から呼ぶ**（pull型）。
  よって「1フレーム分の処理」を引数付き関数として公開することが必須（§3）。FPS制限の `usleep` は削除し、駆動側の責務にする。
...
**✅ 実装済み。ただし設計時から差分があります**（正本は
[`codes/includes/platform/sim.h`](../../codes/includes/platform/sim.h)）。

| 差分 | 実装 | 理由 |
|---|---|---|
| **API が 1 つ増えた** | `game_set_input_source(game, id, source)` を追加 | 切断席を AI へ差し替える（②の再接続仕様）ために、席の入力源を試合中に変更できる必要があった |
| **JS 用の薄いラッパを追加** | `sim_create(...)` / `sim_set_input(..., yaw)` | 構造体ポインタを JS から組み立てずに済ませるため。`mv`/`yaw` → `t_input` の写像は platform/headless の責務（② §6-B） |
| **`game_snapshot` は構造体ではなくフラットな f64 配列を返す** | ヘッダ 5 個 + 戦闘員 1 人あたり 9 個 | WASM 境界で構造体レイアウトを共有せずに済み、JS 側が `HEAPF64` のまま読める。**JSON 化と tick 番号の付与は Node の責務**（② §5-B） |
| **`game_apply_snapshot` は sim 層ではなくクライアント側にある** | `codes/srcs/platform/web/web_snapshot.c` | サーバはこの関数を使わない（`sim.wasm` にリンクしない）。補間は `web/snapshot_interp.js` が担当 |
| **`match_rules` に `seed` を追加** | 非 0 で乱数系列を固定 | 同じ入力列に対して試合を決定的に再現するため（デモ記録と W-10 の結合テストで必須） |
...
**採用に伴う具体的変更点**（ロジック仕様。担当: Engine が構造、Gameplay がルール — 実際は両方 samatsum）:

1. 戦闘員構造に「入力源種別（AI / EXTERNAL）」と「保持する `t_input`」を追加。
   `update_enemies` のループは「AI なら AI が `t_input` を生成 → 共通の移動適用」に再構成。
2. 移動適用は `move_camera` 系（X/Y軸分離・壁ずり・`WALL_MARGIN`）に一本化し、
   敵の `step_enemy` もこれを使う（当たり半径は戦闘員パラメータ化:
   `PLAYER_RADIUS`/`ENEMY_RADIUS` を統合）。
3. 描画時、「自分以外の戦闘員」は既存の敵スプライト描画（8方向/ハンド表示）をそのまま使う。「自分」はカメラになるので描画されない（現行と同じ）。
4. 死亡演出 `death_timer`・`spawn` は戦闘員ごとに持つ（現行はプレイヤー1人分のみ）。

> **✅ 4 項目とも実装済み**（G-01〜G-04）。
> プレイヤーは `world.enemies` リストの 1 ノードになり、AI 席と人間席の違いは
> `input_source`（`AI` / `EXTERNAL`）だけになりました。「担当: Engine が構造、Gameplay がルール」は
> 当初の分担案で、実際は samatsum が両方を実施しています（[6-チーム分担計画](./6-チーム分担計画.md)）。
...
- クライアントの `render_frame` は「スナップショット2つの補間結果を書いた表示用 `t_game`」を読むだけ。
  **クライアントに勝敗判定コードは存在しない**（サーバ権威の徹底）。

> **✅ `tick` / `match` / `combatants[N]` は実装済み**。ただし配線が設計と一部違います。
>
> - `tick` は sim が持ちません。**tick 番号の採番は Node（サーバ）の所有物**です
>   （② §5-C）。sim が返すのはヘッダ 5 個 + 戦闘員 × 9 個のフラットな f64 配列だけです。
> - **`world_delta` だけ未実装です**（唯一の意図的な後送り）。
>   ゲート2 の RSP 2vs2 に不要なため見送りました。FPS のオンライン化（W-14）を行う際に
>   「収集座標リストの蓄積 + 初回全量送信」を追加します
>   （[3-エンジンPhase3レポート](../03_実装レポート/3-エンジンPhase3レポート.md) の申し送り №2）。
> - 「1KB 未満/回」と「クライアントに勝敗判定コードなし」は達成しています。
...
| テクスチャの契約キー | **`.cub` 由来のテクスチャパス文字列**を正式契約とする（⑤ [BACKLOG.md](./5-バックログ.md) D-16。当初の「`t_texture_id` を ID 空間にする」案は不採用）。web は変換時に生成する `manifest.json` のパスキーと `./` 正規化で照合し、native は従来どおり `t_tex.path` を直接読む |

### 4-B. `.cub` の入力経路

- 現行: `open` + gnl（fd 依存）。サーバ/ブラウザではファイルシステムに依存したくない。
- **決定**: パーサの読み取り単位を「fd から1行」→「メモリ上テキストから1行」に差し替える薄いリーダ抽象を入れる。
  native は「ファイルを全読みしてから同じメモリリーダに流す」形に統一（gnl は初期読み込みのみに残存可）。
- マップ検証ロジック（壁閉じ・文字種・スポーン数）は**サーバ起動時に一度だけ**実行。クライアントはパース済み結果を信頼する。

### 4-C. ゲームルール変更（Gameplay レーンの範囲・sim 層のみで完結）
...
> **✅ 5 項目とも実装済み**（G-05 / G-06 / G-07 / G-08 / G-10）。実装時に決まった細部:
>
> - **`target_score` の下限は 1**。「既定 10」は `RSP_SCORE_LIMIT`（0 以下を渡したときの既定値）です。
>   製品仕様の 3〜21 という範囲はエンジンでは検証しません。**範囲チェックは WS 層の責務**
>   （② W-11 №6）で、エンジンは機構としてテスト用の短い試合（N=1、N=2）も回します。
> - **マップ文字 `G` は新設していません**。ゴール判定（`IS_GOAL` セル + `goal_tex`）は
>   もともと実装済みだったため（§0 の表を参照）、G-06 は「勝者を戦闘員 ID に帰属させる」
>   だけで済みました。
> - 死亡した席が同じ tick でゴール・収集できてしまう不具合をレビューで指摘され、修正済みです
>   （`death_timer > 0` の席は `check_quest` の対象外）。回帰テストで固定しています。
> - 上記は [`codes/tests/sim_test.c`](../../codes/tests/sim_test.c)（85 検査）が受入条件を
>   自動検証しており、`make test` で実行できます。
...
> **✅ 実装済み**（E-05 / E-11 / E-14）。CI は
> [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml) にあり、
> 2026-07-23 に全ジョブ green を確認しました。3 ターゲットのビルドに加えて `make check`（lint）・`make test`（85 検査）・
> xvfb 上での native 起動スモークまで自動実行します。
>
> Makefile には設計時になかった追加が 2 点あります。
> ヘッダ依存の自動生成（`-MMD -MP`。古い `.o` が混ざって落ちる事故を防ぐ）と、
> `make test` ターゲット（emcc 不要で gcc 直リンク）です。
...
> **✅ S1〜S7 すべて完了しました。**
> 「目安」列は着手前に引いた日程で、実績ではありません。
> 各行の完了条件は全て満たしており、**native は全期間を通じて 1 度も壊していません**
> （各 Step 後に `maps/fps_map/1.cub` と `maps/rsp_map/rsp.cub` の起動確認を実施）。
...
| ✅ S6 | 戦闘員統合（§3-C）: 入力源の抽象化・移動一本化・戦闘員別 death/spawn | native の RSP/FPS が回帰なし（プレイ感が変わらない）。AI 3席+外部入力1席の混在テストが通る | Day 4–5（S5と並行、Gameplay が合流） |
| ✅ S7 | `game_apply_snapshot` + クライアント側の補間受け口 | 「Node 上の sim.wasm → JSON → ブラウザの render.wasm」の一方通行デモが成立 | Day 5–6 |

**回帰確認の基準**: 各 Step 後に native で `maps/fps_map/1.cub` と `maps/rsp_map/rsp.cub` を起動し、
既存 USER_DOC §2〜5 の操作・ルールが変わっていないことを確認する（`make check` の lint ゲートも維持）。
...
> **✅ 24 件すべてクローズ済み**（E-01〜E-14 / G-01〜G-10）。以下の表は着手前の計画で、
> 「受入条件」「依存」「目安」の列は当時のものです。**残っている Issue はありません。**
>
> - 「Engine 担当」「Gameplay 担当」は当初の 4 レーン体制での分担案です。
>   実際には samatsum が単独で両レーンを完了させました。
>   現在有効な担当表は [6-チーム分担計画](./6-チーム分担計画.md) です。
> - 各 Issue の実装内容と検証結果は
>   [03_実装レポート](../03_実装レポート/) の 4 本にまとめてあります。

### Engine レーン担当（実際は samatsum）
...
### Gameplay レーン担当（実際は samatsum）

| # | タイトル | 受入条件 | 依存 | 目安 |
|---|---|---|---|---|
| ✅ G-01 | 戦闘員統合の仕様レビュー（§3-C の4項目の詳細化） | Engine 側と合意した変更仕様書（構造体フィールド表）が確定 | E-04 | 0.5日 |
...
**クリティカルパス**: E-01→E-02→(E-03/E-04)→E-05→E-07（ゲート1）→E-08 と、E-09→E-10→E-11→E-12。
G 系は E-04 合意後に並行着手できる。
Engine の合計 ≒ 8.5人日、Gameplay の合計 ≒ 6.5人日で、Day 1〜9 の2レーンに収まる（バッファ含む）。

> **✅ このクリティカルパスは踏破済みです。** ゲート1（E-07）は 2026-07-11 に通過し、
> 最後の E-13 は 2026-07-23 に完了しました。
...
| 日付 | 内容 |
|---|---|
| 2026-07-11 | §2・§4-A: `pf_load_texture` を ID 契約から**パス契約**へ改訂（⑤ [BACKLOG.md](./5-バックログ.md) D-16。GATE1 実装の正式化。選択肢比較は ⑤ §0） |
| 2026-07-23 | 全 24 Issue（E-01〜E-14 / G-01〜G-10）の完了に伴い、**実装状況を各節へ明示**。§3-B に実装との差分表（`game_set_input_source` の追加・フラット f64 スナップショット・`seed`）、§3-D に `world_delta` 未実装の明記、§4-C に `target_score` 下限とマップ文字 `G` 不採用を追記。長い行を折り返して可読性を改善 |
