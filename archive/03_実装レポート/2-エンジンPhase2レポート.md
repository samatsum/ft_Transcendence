...
| E-08 追補 | OK | **武器切替（1/2/3）と射撃（Space）を web に接続**（ユーザー指摘による受入条件の追加）。native のキー処理から `select_weapon`/`trigger_shot` を公開関数へ切り出し、web の `web_set_weapon`/`web_shoot` と同じ実体を共有（クールダウン・can_shoot 制約も C 側で一元管理）。ヘッドレス検証: 4状態（ピストル/ライト/素手/射撃）で描画オーバーレイの変化を確認 |
